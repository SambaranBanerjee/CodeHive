...
export default function SignupPage() {
  ...
  const scoreRef = useRef(0); // ✅ real-time score tracking
  ...
  const runGameLoop = useCallback(() => {
    ...
    function checkCoinCollection() {
      ...
      coins.current = coins.current.filter((c) => {
        ...
        if (collected) {
          scoreRef.current += 1; // ✅ live increment
          setScore(scoreRef.current); // sync with React
    ...
    function update() {
      ...
      // Draw score + highscore (top right)
      if (!ctx) return;
      ctx.fillStyle = "white";
      ctx.font = "20px Arial";
      if (!canvas) return;
